...
class WashingMachineSlotStorageImplementation(
    ...
    def washing_machines_in_given_slot(self, day: str, start_time: str, 
        ...
        if wm_objs:
            for wm_obj in wm_objs:
                washing_machine.append(WashingMachineDto(
                    wm_id=wm_obj.washing_machine_id)
                )

            return washing_machine

        invalid_wm_start_time_objs = WashingmachineSlot.objects.filter(
                day=day, start_time=start_time
        )

        if invalid_wm_start_time_objs:
            washing_machine.append(WashingMachineDto(
                wm_id="start_time")
            )
            return washing_machine
